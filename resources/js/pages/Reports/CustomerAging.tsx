                                                ...
                                                {data.map(c => (
                                                    <>
                                                        <tr key={c.customer_id} className="hover:bg-black/3 dark:hover:bg-white/3 transition-colors">
                                                            ...
                                                            <td className="px-4 py-3 text-center font-bold text-slate-500 dark:text-white/50">{c.invoices.length}</td>
                                                        ...
                                                        {expanded.has(c.customer_id) && (
                                                            <tr key={`${c.customer_id}-detail`}>
                                                                <td colSpan={8} className="px-4 py-2 bg-black/2 dark:bg-white/2">
                                                                    <table className="w-full text-xs">
                                                                        <thead>
                                                                            <tr className="text-slate-400 dark:text-white/30">
                                                                                <th className="text-right py-1 px-2 font-black">رقم الفاتورة</th>
                                                                                <th className="text-right py-1 px-2 font-black">التاريخ</th>
                                                                                <th className="text-right py-1 px-2 font-black">الإجمالي</th>
                                                                                <th className="text-right py-1 px-2 font-black">المدفوع</th>
                                                                                <th className="text-right py-1 px-2 font-black">المتبقي</th>
                                                                                <th className="text-right py-1 px-2 font-black">العمر</th>
                                                                            </tr>
                                                                        </thead>
                                                                        <tbody className="divide-y divide-black/5 dark:divide-white/5">
                                                                            {c.invoices.map(inv => (
                                                                                <tr key={inv.invoice_id}>
                                                                                    <td className="py-1.5 px-2 font-black text-primary">INV#{inv.invoice_id}</td>
                                                                                    <td className="py-1.5 px-2 text-slate-500 dark:text-white/50">{inv.date.slice(0, 10)}</td>
                                                                                    <td className="py-1.5 px-2 font-bold text-slate-700 dark:text-white/70">{fmt(inv.total)}</td>
                                                                                    <td className="py-1.5 px-2 font-bold text-emerald-600 dark:text-emerald-400">{fmt(inv.paid)}</td>
                                                                                    <td className="py-1.5 px-2 font-black text-slate-800 dark:text-white">{fmt(inv.due)}</td>
                                                                                    <td className={`py-1.5 px-2 ${agingClass(inv.days_old)}`}>{inv.days_old} يوم</td>
                                                                                </tr>
                                                                            ))}
                                                                        </tbody>
                                                                    </table>
                                            ...
                                            <div key={c.customer_id} className="rounded-[20px] border border-black/8 dark:border-white/12 overflow-hidden">
                                                ...
                                                <div className="px-4 py-2 flex flex-col gap-1.5 text-sm">
                                                    ...
                                                    <button onClick={() => toggleExpand(c.customer_id)}
                                                        ...
                                                        <ChevronRight className={`w-3.5 h-3.5 transition-transform ${expanded.has(c.customer_id) ? 'rotate-90' : ''}`} />
                                                        {expanded.has(c.customer_id) ? 'إخفاء الفواتير' : `${c.invoices.length} فاتورة`}
                                                    </button>
                                                    {expanded.has(c.customer_id) && c.invoices.map(inv => (
                                                        <div key={inv.invoice_id} className="flex justify-between text-xs border-t border-black/5 dark:border-white/5 pt-1.5 mt-0.5">
                                                            <span className="font-black text-primary">INV#{inv.invoice_id}</span>
                                                            <span className="font-bold text-slate-500 dark:text-white/50">{inv.date.slice(0, 10)}</span>
                                                            <span className="font-black text-slate-800 dark:text-white">{fmt(inv.due)}</span>
                                                            <span className={agingClass(inv.days_old)}>{inv.days_old}ي</span>
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
