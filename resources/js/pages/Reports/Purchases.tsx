...
interface Props {
    ...
    filters: { dateFrom: string | null; dateTo: string | null; userId: number | null; supplierId: number | null; categoryId: number | null; compare: boolean; };
    data: PurchasesData;
}
...
export default function Purchases({ users, suppliers, categories, filters, data }: Props) {
    ...
    const hasFilter = dateFrom || dateTo || userId || supplierId || categoryId || compare;
    ...
    function reset() {
        setDateFrom(''); setDateTo(''); setUserId(''); setSupplierId(''); setCategoryId(''); setCompare(false);
    ...
    return (
        <AppShell pageTitle="تقرير المشتريات">
            <div className="flex flex-col gap-6 pb-32 lg:pb-6">
                <div>
                    <h1 className="text-2xl font-black text-slate-800 dark:text-white">تقرير المشتريات</h1>
                    <p className="text-sm font-bold text-slate-400 dark:text-white/40 mt-1">تحليل المشتريات اليومي والشهري</p>
                </div>
                    ...
                    <div className="flex-1 min-w-0 flex flex-col gap-6">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                            <div className="spatial-card p-4 flex flex-col gap-1">
                                <p className="text-xs font-black text-slate-400 dark:text-white/40 uppercase tracking-widest">إجمالي المشتريات</p>
                                <p className="text-2xl font-black text-slate-800 dark:text-white">{fmt(data.totalPurchases)}</p>
                                {data.comparison && (
                            ...
                            <div className="spatial-card p-4 flex flex-col gap-1">
                                ...
                                <p className="text-2xl font-black text-slate-800 dark:text-white">{data.purchasesCount}</p>
                                {data.comparison && (
                                    <p className="text-xs font-bold text-slate-400 dark:text-white/30 mt-1">سابق: {data.comparison.purchases_count}</p>
                                )}
                            </div>
                            <div className="spatial-card p-4 flex flex-col gap-1">
                                <p className="text-xs font-black text-slate-400 dark:text-white/40 uppercase tracking-widest">متوسط الفاتورة</p>
                                <p className="text-2xl font-black text-slate-800 dark:text-white">{fmt(data.avgPurchase)}</p>
                            </div>
                            <div className="spatial-card p-4 flex flex-col gap-1">
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <a href={buildSupplierInvoicesUrl()}
                                ...
                                <Truck className="w-4 h-4" /> فواتير الموردين
                            </a>
                        </div>

                        <SpatialCard title={`التفصيل الشهري (${data.monthly.length} شهر)`} icon={<ShoppingBag className="w-4 h-4" />}>
                                                                ...
                                                                <table className="w-full text-[15px]">
                                                                    <thead>
                                                                        <tr className="border-b border-black/5 dark:border-white/5">
                                                                            ...
                                                                            <th className="text-right py-3 px-4 font-black text-slate-500 dark:text-white/40 text-sm uppercase tracking-widest">إجمالي المشتريات</th>
                                                                        </tr>
                                                                    </thead>
                                                                    <tbody className="divide-y divide-black/5 dark:divide-white/5">
                                                                        {m.days.map((d, i) => (
