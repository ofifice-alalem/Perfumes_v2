    ...
    return (
        <AppShell pageTitle="العملاء">
            <div className="flex flex-col gap-6 pb-32 lg:pb-0">
                ...
                {/* List */}
                <SpatialCard title={`العملاء (${customers.length})`} icon={<Users className="w-4 h-4" />}>
                    {customers.length === 0 ? (
                                    ...
                                    <tbody className="divide-y divide-black/5 dark:divide-white/5">
                                        {customers.map(customer => (
                                            editingId === customer.id ? (
                                                ...
                                                <tr key={customer.id} className="hover:bg-black/3 dark:hover:bg-white/3 transition-colors">
                                                    <td className="px-4 py-3 font-bold text-slate-800 dark:text-white">{customer.name}</td>
                                                    <td className="px-4 py-3 font-bold text-slate-500 dark:text-white/60 whitespace-nowrap">{customer.phone ?? '—'}</td>
