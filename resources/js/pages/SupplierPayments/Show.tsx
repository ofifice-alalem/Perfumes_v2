...
import { CreditCard, Truck, FileText, Calendar, MessageSquare } from 'lucide-react';

interface Payment {
  id: number; amount: string; notes: string | null; created_at: string;
  supplier: { id: number; name: string; total_debt: string; total_purchases: string } | null;
  purchase: { id: number; total: string; payment_status: string } | null;
  ...
  return (
    <span className="font-black text-slate-800 dark:text-white text-sm text-left">
      {parts.map((part, i) => {
        ...
        return isNum && prev.includes('الشراء')
          ? <Link key={i} href={`/purchases/${part}`} className="text-primary underline hover:opacity-75 transition-opacity">{part}</Link>
...
export default function SupplierPaymentShow({ payment }: Props) {
  const debt = payment.supplier ? +payment.supplier.total_debt : 0;
      ...
      <div className="flex flex-col gap-6 pb-32 lg:pb-0">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <Link href="/supplier-payments" className="text-slate-400 dark:text-white/40 hover:text-primary transition-all font-bold text-sm">← مدفوعات الموردين</Link>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {payment.supplier && (
            <div className="flex flex-col gap-6">
              <SpatialCard title="المورد" icon={<Truck className="w-4 h-4" />}>
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex items-center gap-4 p-4 rounded-[16px] bg-black/3 dark:bg-white/3">
                    <div className="w-14 h-14 rounded-full bg-primary/10 flex items-center justify-center font-black text-2xl text-primary shrink-0">
                      {payment.supplier.name.charAt(0)}
                    </div>
                    <div>
                      <span className="font-black text-slate-800 dark:text-white text-lg">{payment.supplier.name}</span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    <div className="p-4 rounded-[16px] bg-black/3 dark:bg-white/3 border border-black/5 dark:border-white/5">
                      ...
                      <div className="flex items-end gap-1 mt-2">
                        <span className="text-3xl font-black text-slate-800 dark:text-white">{payment.supplier.total_purchases}</span>
                    ...
                    <div className={`p-4 rounded-[16px] border ${
                      ...
                      <span className={`text-xs font-bold uppercase tracking-widest ${
                        ...
                        {debt > 0 ? 'دين' : debt < 0 ? 'دائن' : 'الحالة المالية'}
                      </span>
                      <div className="flex items-end gap-1 mt-2">
                  ...
                  <Link href="/suppliers"
                    ...
                    <Truck className="w-4 h-4" /> عرض الموردين
                  </Link>
                </div>
              </SpatialCard>

              {/* فاتورة الشراء المرتبطة تحت المورد */}
              {payment.purchase && (
                <SpatialCard title="فاتورة الشراء المرتبطة" icon={<FileText className="w-4 h-4" />}>
                  <div className="flex items-center justify-between p-4 rounded-[16px] bg-black/3 dark:bg-white/3 border border-black/5 dark:border-white/5">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <span className="font-black text-slate-800 dark:text-white">فاتورة شراء #{payment.purchase.id}</span>
                        <p className="text-sm font-bold text-slate-400 dark:text-white/40 mt-0.5">إجمالي: {payment.purchase.total} د</p>
                      </div>
                    </div>
