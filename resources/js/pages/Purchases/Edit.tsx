...
interface Purchase { id: number; notes: string | null; supplier: { name: string }; }
interface Props { purchase: Purchase; flash?: { success?: string; error?: string }; }

export default function PurchasesEdit({ purchase, flash }: Props) {
    const form = useForm({ notes: purchase.notes ?? '', items: [] });
    ...
    return (
        <AppShell pageTitle={`تعديل فاتورة #${purchase.id}`}>
            <div className="flex flex-col gap-6 pb-32 lg:pb-0">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-black text-slate-800 dark:text-white">تعديل فاتورة #{purchase.id}</h1>
                ...
                <SpatialCard title="تعديل الملاحظات">
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <Link href={`/purchases/${purchase.id}`}
                                className="h-11 px-5 rounded-[20px] bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 text-slate-600 dark:text-white/60 font-bold text-sm transition-all flex items-center">
                                إلغاء
                            </Link>
                        </div>
                    </div>
