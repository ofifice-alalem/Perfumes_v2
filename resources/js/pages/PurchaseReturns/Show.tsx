            ...
            <div className="flex flex-col gap-6 pb-32 lg:pb-0">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    {[
                        ...
                        { label: 'الفاتورة المرجعية', value: ret.purchase ? `#${ret.purchase.id}` : 'مستقل', color: 'text-slate-800 dark:text-white' },
                    ].map(s => (
                        <div key={s.label} className="spatial-card p-4 flex flex-col gap-1">
                            ...
                            {s.label === 'الفاتورة المرجعية' && ret.purchase ? (
                                <Link href={`/purchases/${ret.purchase.id}`} className={`text-xl font-black ${s.color} hover:underline`}>{s.value}</Link>
                            ) : (
                ...
                <SpatialCard title={`الاسترداد (${ret.settlements.length})`} icon={<RefreshCw className="w-4 h-4" />}
                    action={
                        !isCancelled && !isCash && ret.recovery_status !== 'paid' && (
                            <button onClick={() => { setShowForm(p => !p); setRows([emptyRow()]); }}
                                ...
                                <Plus className="w-3.5 h-3.5" /> تسجيل استرداد
                            </button>
                        )
                    }
                >
                    ...
                    {showForm && (
                        <div className="mb-5 p-4 rounded-[20px] bg-purple-500/5 border border-purple-500/20 flex flex-col gap-4">
                            <div className="flex items-center justify-between">
                                <span className="font-black text-slate-700 dark:text-white/80 text-sm">تسجيل استرداد جديد</span>
                            ...
                            {rows.length > 1 && (
                                <div className="flex items-center justify-between px-2">
                                    <span className="font-bold text-slate-500 dark:text-white/50 text-sm">إجمالي هذا الاسترداد</span>
