...
import { Plus, Trash2, CreditCard, X, Check } from 'lucide-react';
...
function NewPaymentForm({ customers, paymentMethods, onClose }: {
  ...
  function submit() {
    ...
    setProcessing(true);
    router.post('/payments', form, { onSuccess: onClose, onFinish: () => setProcessing(false) });
  }
...
export default function PaymentsIndex({ payments, customers, paymentMethods, filters, flash }: Props) {
  ...
  function applyFilters() {
    router.get('/payments', localFilters, { preserveState: true });
  }
  ...
  function deletePayment(id: number) {
    router.delete(`/payments/${id}`, { onSuccess: () => setDeleteId(null) });
  }
      ...
      <div className="flex flex-col gap-6 pb-32 lg:pb-0">
        ...
        {showCreate && (
          <SpatialCard title="دفعة جديدة" icon={<Plus className="w-4 h-4" />}>
            <NewPaymentForm customers={customers} paymentMethods={paymentMethods} onClose={() => setShowCreate(false)} />
          </SpatialCard>
        )}

        {/* Filters */}
        <SpatialCard title="فلاتر البحث" hideHeader={false}>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            ...
            <input type="date" value={localFilters.date_from ?? ''} onChange={e => setLocalFilters(f => ({ ...f, date_from: e.target.value }))}
              className="spatial-input h-10 rounded-[12px] px-3 text-sm font-bold" placeholder="من تاريخ" />
            <input type="date" value={localFilters.date_to ?? ''} onChange={e => setLocalFilters(f => ({ ...f, date_to: e.target.value }))}
              className="spatial-input h-10 rounded-[12px] px-3 text-sm font-bold" placeholder="إلى تاريخ" />
            <button onClick={applyFilters}
              className="spatial-button h-10 text-sm font-bold">بحث</button>
          ...
          {payments.data.length === 0 ? (
            ...
            <div className="flex flex-col gap-2">
              ...
              {payments.data.map(payment => (
                <div key={payment.id}>
                  {/* Desktop row */}
                  <div className="hidden sm:grid grid-cols-[2fr_1fr_1fr_1fr_80px] gap-3 px-4 py-3 rounded-[16px] bg-black/3 dark:bg-white/3 border border-black/5 dark:border-white/5 hover:border-primary/20 transition-all items-center">
                    ...
                    <span className="text-sm font-bold text-slate-600 dark:text-white/70">{payment.payment_method.name}</span>
                  ...
                  <div className="sm:hidden p-4 rounded-[16px] bg-black/3 dark:bg-white/3 border border-black/5 dark:border-white/5">
                    <div className="flex items-center justify-between mb-2">
                      <Link href={`/payments/${payment.id}`} className="font-bold text-slate-800 dark:text-white hover:text-primary">
                        {payment.customer?.name ?? 'زبون نقدي'}
                      </Link>
                      <span className="font-black text-emerald-600 dark:text-emerald-400">{payment.amount} د</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-bold text-slate-400 dark:text-white/40">{payment.payment_method.name}</span>
          ...
          {payments.last_page > 1 && (
            <div className="flex items-center justify-center gap-2 mt-4 pt-4 border-t border-black/5 dark:border-white/5">
              {Array.from({ length: payments.last_page }, (_, i) => i + 1).map(page => (
                <button key={page}
                  onClick={() => router.get('/payments', { ...filters, page: String(page) })}
