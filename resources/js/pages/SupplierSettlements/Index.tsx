...
export default function SettlementsIndex({ settlements, suppliers, paymentMethods, filters, flash }: Props) {
  ...
  const selectedSupplier = suppliers.find(c => String(c.id) === form.supplier_id);
  const isDebtor = selectedSupplier ? +selectedSupplier.total_debt > 0 : false;
  ...
  function applyFilters() {
    ...
    router.get('/supplier-settlements', p, { preserveState: true, replace: true });
  }

  function resetFilters() {
    setSupplierId(''); setPaymentMethodId(''); setDateFrom(''); setDateTo('');
    setAmountMin(''); setAmountMax('');
    router.get('/supplier-settlements', {}, { preserveState: true, replace: true });
  }

  function submitCreate() {
    if (!form.supplier_id || !form.payment_method_id || !form.amount) return;
    setProcessing(true);
    router.post('/supplier-settlements', form, {
  ...
  const filterContent = (
    <div className="flex flex-col gap-4">
      <ModernSelect label="المورد" placeholder="الكل"
        ...
        options={[{ label: 'الكل', value: '' }, ...suppliers.map(c => ({ label: c.name, value: String(c.id) }))]}
      ...
      <div className="flex flex-col gap-6 pb-32 lg:pb-0">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm font-bold text-slate-400 dark:text-white/40 mt-1">{settlements.total} تسوية للموردين</p>
          </div>
          ...
          <SpatialCard title="تسوية جديدة" icon={<Plus className="w-4 h-4" />}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <ModernSelect label="المورد" placeholder="اختر المورد..."
                defaultValue=""
                options={suppliers.filter(c => +c.total_debt < 0).map(c => ({ label: c.name, value: String(c.id) }))}
            ...
            {selectedSupplier && (
              isDebtor ? (
                <div className="mt-4 px-4 py-3 rounded-[14px] bg-red-500/10 border border-red-500/20 text-red-600 dark:text-red-400 font-bold text-sm">
                  المورد مدين بـ {selectedSupplier.total_debt} د — لا يمكن إنشاء تسوية، استخدم الدفعات
                </div>
              ) : (
                <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {+selectedSupplier.total_debt < 0 && (
                    <div className="sm:col-span-2 px-4 py-3 rounded-[14px] bg-emerald-500/10 border border-emerald-500/20 text-emerald-700 dark:text-emerald-300 font-bold text-sm">
                      المتجر دائن لهذا المورد بـ {Math.abs(+selectedSupplier.total_debt)} د
                    </div>
                  )}
            ...
            <div className="flex items-center gap-2 mt-4">
              <button onClick={submitCreate}
                disabled={processing || !form.supplier_id || !form.payment_method_id || !form.amount || isDebtor}
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-4">
            ...
            {settlements.data.length === 0 ? (
              ...
              settlements.data.map(settlement => (
                <div key={settlement.id} className="spatial-card overflow-hidden flex flex-col sm:flex-row sm:items-stretch">

                  {/* Color strip - desktop only */}
                  <div className="hidden sm:flex sm:flex-col sm:w-28 shrink-0 items-center sm:justify-center sm:px-3 sm:py-0 border-l border-black/5 dark:border-white/5 bg-amber-500/10 text-amber-600 dark:text-amber-400">
                    <span className="text-[13px] font-black">{settlement.amount} د</span>
                  </div>
                  ...
                  <div className="hidden sm:flex flex-1 flex-row items-center min-w-0 px-4 py-3 gap-5">
                    <div className="flex flex-col gap-1 flex-1 min-w-0">
                      <div className="flex items-center gap-3">
                        <span className="text-[15px] font-black text-primary">#{settlement.id}</span>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {settlement.purchase && (
                          <Link href={`/purchases/${settlement.purchase.id}`}
                            className="text-xs font-bold text-primary/70 hover:text-primary transition-colors">
                            فاتورة شراء #{settlement.purchase.id}
                          </Link>
                        )}
                        {settlement.notes && (
                          <span className="text-xs font-bold text-slate-400 dark:text-white/30 truncate">{settlement.notes}</span>
                        )}
                      </div>
                    </div>
                    <div className="flex items-center gap-3 shrink-0">
                      <Link href={`/supplier-settlements/${settlement.id}`}
                  ...
                  <div className="sm:hidden p-5 flex flex-col gap-4">
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="font-black text-slate-800 dark:text-white text-base">{settlement.supplier?.name ?? '—'}</div>
            ...
            {settlements.last_page > 1 && (
              <div className="flex items-center justify-between pt-2">
                <span className="text-xs font-bold text-slate-400 dark:text-white/40">
                  صفحة {settlements.current_page} من {settlements.last_page}
                </span>
                <div className="flex items-center gap-2">
                  <button onClick={() => goToPage(settlements.links[0].url)} disabled={settlements.current_page === 1}
                  ...
                  {settlements.links.slice(1, -1).map((link, i) => (
                  ...
                  <button onClick={() => goToPage(settlements.links[settlements.links.length - 1].url)} disabled={settlements.current_page === settlements.last_page}
                    className="w-9 h-9 rounded-[12px] spatial-input flex items-center justify-center text-slate-600 dark:text-white/60 disabled:opacity-30 hover:text-primary transition-colors">
        ...
        <ConfirmModal isOpen={deleteId !== null} title="حذف التسوية"
          message="هل أنت متأكد من حذف هذه التسوية؟ سيُعاد حساب دين المورد تلقائياً."
          onConfirm={() => deleteId && router.delete(`/supplier-settlements/${deleteId}`, { onSuccess: () => setDeleteId(null) })}
          onCancel={() => setDeleteId(null)} />

        <NumberPadModal isOpen={showPad} onClose={() => setShowPad(false)} title="المبلغ"
          initialValue={form.amount}
          onConfirm={v => {
            const max = selectedSupplier ? Math.abs(+selectedSupplier.total_debt) : 0;
