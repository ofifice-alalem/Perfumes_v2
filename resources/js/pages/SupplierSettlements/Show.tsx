...
import { ArrowLeftRight, Truck, FileText, Calendar, MessageSquare } from 'lucide-react';
...
interface Settlement {
  ...
  purchase: { id: number; total: string } | null;
  payment_method: { name: string };
}
interface Props { settlement: Settlement; }

export default function SettlementShow({ settlement }: Props) {
  const debt = settlement.supplier ? +settlement.supplier.total_debt : 0;
      ...
      <div className="flex flex-col gap-6 pb-32 lg:pb-0">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <Link href="/supplier-settlements" className="text-slate-400 dark:text-white/40 hover:text-primary transition-all font-bold text-sm">← تسويات الموردين</Link>
              ...
              <SpatialCard title="فاتورة الشراء المرتبطة" icon={<FileText className="w-4 h-4" />}>
                <div className="flex items-center justify-between p-4 rounded-[16px] bg-black/3 dark:bg-white/3 border border-black/5 dark:border-white/5">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <span className="font-black text-slate-800 dark:text-white">فاتورة شراء #{settlement.purchase.id}</span>
                      <p className="text-sm font-bold text-slate-400 dark:text-white/40 mt-0.5">إجمالي: {settlement.purchase.total} د</p>
                    </div>
                  </div>
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex items-center gap-4 p-4 rounded-[16px] bg-black/3 dark:bg-white/3">
                    <div className="w-14 h-14 rounded-full bg-amber-500/10 flex items-center justify-center font-black text-2xl text-amber-600 dark:text-amber-400 shrink-0">
                      {settlement.supplier.name.charAt(0)}
                    </div>
                    <div>
                      <span className="font-black text-slate-800 dark:text-white text-lg">{settlement.supplier.name}</span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    <div className="p-4 rounded-[16px] bg-black/3 dark:bg-white/3 border border-black/5 dark:border-white/5">
                      ...
                      <div className="flex items-end gap-1 mt-2">
                        <span className="text-3xl font-black text-slate-800 dark:text-white">{settlement.supplier.total_purchases}</span>
